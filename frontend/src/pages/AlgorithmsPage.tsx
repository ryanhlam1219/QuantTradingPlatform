...
import { StrategyVisualizer } from "../components/algorithms/StrategyVisualizer";
...
export function AlgorithmsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"overview"|"viz"|"params"|"tips">("overview");
      ...
      <div className="algo-layout">
        ...
        <div className="algo-detail">
          ...
          <div className="algo-tabs">
            {(["overview","viz","params","tips"] as const).map(t => (
              <button key={t}
                className={`algo-tab ${activeTab === t ? "active" : ""}`}
                style={activeTab===t ? {"--tab-accent": color} as any : undefined}
                onClick={() => setActiveTab(t)}
              >
                {t === "overview" ? "📖 Overview" : t === "viz" ? "📊 Visualise" : t === "params" ? "⚙ Parameters" : "💡 Tuning Tips"}
              </button>
            ))}
          </div>
          ...
          {activeTab === "viz" && (
            <div className="algo-section">
              <h3 className="algo-section-title">Interactive Signal Visualizer</h3>
              <p className="algo-section-body" style={{ marginBottom: 16 }}>
                Drag the sliders to change parameters and watch how the buy/sell signals shift in real time.
                Data is synthetic but realistic — the logic is identical to what runs in the backtest engine.
              </p>
              <StrategyVisualizer strategy={selected} />
            </div>
          )}
